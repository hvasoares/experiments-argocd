...
- [ ] T032 Apply `bootstrap/root-application.yaml` to the cluster from T011 and confirm `platform-addons`/`team-addons` reach `Synced`/`Healthy` (quickstart.md steps 2-3) (depends on: T011, T015, T016, T022, T028)
- [ ] T033 Confirm `ingress-nginx` and `postgresql-platform` reach `Synced`/`Healthy` on the live cluster (quickstart.md step 4) (depends on: T032)
- [ ] T034 Confirm `postgresql-team` and `outline` reach `Synced`/`Healthy`, curl Outline via its ingress hostname, and confirm it resolves against `postgresql-team` (not `postgresql-platform`) live, not just statically (quickstart.md steps 5-6) (depends on: T032, T033)
- [ ] T035 Tear down the local cluster: `kind delete cluster --name argocd-playground` (quickstart.md step 7) (depends on: T034)

**BLOCKED (2026-07-07 implementation run)**: T032 fails — `bootstrap/root-application.yaml`'s `source.repoURL` is still the `https://example.com/local-single-repo-placeholder.git` placeholder from T012/T015, which isn't a real git remote Argo CD's repo-server can clone (`example.com` returns an HTML page, not a git response → `ComparisonError: repository not found`). This repo has no `git remote` configured yet. T033-T035 are all downstream of this and cannot proceed until it's fixed. The live `kind-argocd-playground` cluster (Argo CD healthy, `root` Application stuck at `Sync Status: Unknown`) was deliberately left running for debugging rather than torn down. Fix options: (a) push this repo to a real git remote (GitHub/GitLab) and update `repoURL`/`targetRevision` in both `chart/values.yaml` and `bootstrap/root-application.yaml`, or (b) stand up a local git server reachable from the kind cluster's Docker network. Re-run T032-T035 after either fix.
